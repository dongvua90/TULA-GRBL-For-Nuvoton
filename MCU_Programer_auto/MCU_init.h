
//Define Clock source
#define MCU_CLOCK_SOURCE      
#define MCU_CLOCK_SOURCE_HIRC // HXT, LXT, HIRC, LIRC 
#define MCU_CLOCK_FREQUENCY   50000000  //Hz

//Define MCU Interfaces
#define MCU_INTERFACE_TMR0
#define TMR0_CLOCK_SOURCE_HIRC // HXT, LXT, HCLK, EXT, LIRC, HIRC
#define TMR0_CLOCK_DIVIDER    1
#define TMR0_OPERATING_MODE   ONESHOT	//, PERIODIC, TOGGLE, CONTINUOUS TIMER_PERIODIC_MODE // 
#define TMR0_OPERATING_FREQ   65000 //Hz
#define MCU_INTERFACE_TMR1
#define TMR1_CLOCK_SOURCE_HIRC // HXT, LXT, HCLK, EXT, LIRC, HIRC
#define TMR1_CLOCK_DIVIDER    1
#define TMR1_OPERATING_MODE   TIMER_PERIODIC_MODE // ONESHOT, PERIODIC, TOGGLE, CONTINUOUS
#define TMR1_OPERATING_FREQ   65000 //Hz 
#define MCU_INTERFACE_TMR2
//#define TMR2_CLOCK_SOURCE_HIRC // HXT, LXT, HCLK, EXT, LIRC, HIRC
//#define TMR2_CLOCK_DIVIDER    1
//#define TMR2_OPERATING_MODE   TIMER_PERIODIC_MODE // ONESHOT, PERIODIC, TOGGLE, CONTINUOUS
//#define TMR2_OPERATING_FREQ   Z_SPEED //Hz z
//#define MCU_INTERFACE_TMR3
//#define TMR3_CLOCK_SOURCE_HIRC // HXT, LXT, HCLK, EXT, LIRC, HIRC
//#define TMR3_CLOCK_DIVIDER    1
//#define TMR3_OPERATING_MODE   TIMER_PERIODIC_MODE // ONESHOT, PERIODIC, TOGGLE, CONTINUOUS
//#define TMR3_OPERATING_FREQ   Y_SPEED //Hz y

//Define MCU Interfaces
//#define MCU_INTERFACE_UART1
//#define UART_CLOCK_SOURCE_HIRC // HXT, LXT, PLL, HIRC
//#define UART_CLOCK_DIVIDER     3
//#define PIN_UART1_RX_PB4
//#define PIN_UART1_TX_PB5

#define MCU_INTERFACE_UART0
#define UART_CLOCK_SOURCE_HIRC // HXT, LXT, PLL, HIRC
#define UART_CLOCK_DIVIDER     3
#define PIN_UART0_RX_PB0
#define PIN_UART0_TX_PB1

