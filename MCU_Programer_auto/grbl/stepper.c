...
void st_wake_up()
{
  ...
  // Enable stepper drivers.
    ...
    if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) 
		{ 
			STEPPER_ENABLE;	//STEPPERS_DISABLE_PORT |= (1<<STEPPERS_DISABLE_BIT); 
		}else { 
			STEPPER_DISABLE;	//STEPPERS_DISABLE_PORT &= ~(1<<STEPPERS_DISABLE_BIT); 
		}
    // Initialize stepper output bits to ensure first ISR call does not step.
  ...
  //TIMER_EnableInt(TIMER1);	//TIMSK1 |= (1<<OCIE1A);
	//TIMER_Start(TIMER1);
}
...
void st_go_idle()
{
  // Disable Stepper Driver Interrupt. Allow Stepper Port Reset Interrupt to finish, if active.
	//TIMER_DisableInt(TIMER1);	//TIMSK1 &= ~(1<<OCIE1A); // Disable Timer1 interrupt
	//TIMER_Stop(TIMER1);
	TIMER1->TCSR &=~ (TIMER_TCSR_IE_Msk | TIMER_TCSR_CEN_Msk);
	//TIMER1->TCSR = TMR1_OPERATING_MODE | 2;	//TCCR1B = (TCCR1B & ~((1<<CS12) | (1<<CS11))) | (1<<CS10); // Reset clock to no prescaling.
...
uint32_t dbtiktim1=0;
void TMR1_IRQHandler(void)	//ISR(TIMER1_COMPA_vect)
{
  ...
  // Set the direction pins a couple of nanoseconds before we step the steppers
	...
	if((st.dir_outbits & (1<<X_DIRECTION_BIT))>0){
		...
		if((step_port_invert_mask & (1<<Y_DIRECTION_BIT))>0){
			dirY=1;
		}else{
			dirY=0;
		}
		if((step_port_invert_mask & (1<<Z_DIRECTION_BIT))>0){
			dirZ=1;
		}else{
			dirZ=0;
		}
  ...
  // Then pulse the stepping pins
      ...
      //STEP_PORT = (STEP_PORT & ~STEP_MASK) | st.step_outbits;
		if((st.step_outbits & (1<<X_STEP_BIT))>0){
			pX=1;
		}else{
			pX=0;
		}
		if((step_port_invert_mask & (1<<Y_STEP_BIT))>0){
			pY=1;
		}else{
			pY=0;
		}
		if((step_port_invert_mask & (1<<Z_STEP_BIT))>0){
			pZ=1;
		}else{
			pZ=0;
		}
  ...
  // exactly settings.pulse_microseconds microseconds, independent of the main Timer1 prescaler.
	// TIMER1 tao Rising -> sau do ta can tao xung Falling sau 1 khoang thoi gian bang st.step_pulse_time bang TIMER0 
	//PhucEdited: Clear current timer configuration/
    TIMER0->TCMPR = 480;
 ...
 // sei(); // Re-enable interrupts to allow Stepper Port Reset Interrupt to fire on-time.
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      TIMER1->TCMPR = st.exec_segment->cycles_per_tick;//OCR1A = st.exec_segment->cycles_per_tick;
...
uint32_t dbtiktim0;
void TMR0_IRQHandler(void)		//ISR(TIMER0_OVF_vect)
{
	dbtiktim0++;
  // Reset stepping pins (leave the direction pins)
	// STEP_MASK = 0b00011100;
  #ifdef DEFAULTS_RAMPS_BOARD
   ...
   // STEP_PORT = (STEP_PORT & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK);
		if((step_port_invert_mask & (1<<X_STEP_BIT))>0){
			pX=1;
		}else{
			pX=0;
		}
...
void st_reset()
{
  ...
  st_generate_step_dir_invert_masks();
    ...
    //STEP_PORT = (STEP_PORT & ~STEP_MASK) | step_port_invert_mask;		// dat cac Pin_Step = 0
    //DIRECTION_PORT = (DIRECTION_PORT & ~DIRECTION_MASK) | dir_port_invert_mask;	// dat cac Pin direc = 0
		pX=0;
		pY=0;
		pZ=0;
		dirX=0;
...
void stepper_init()
{
	// Cau hinh cac chan xuat dieu khien Stepper da duoc cau hinh trong ControlOutput_Init()
	
	// Cau hinh Timer 0 de tao canh falling
	CLK_EnableModuleClock(TMR0_MODULE);				// CLK_TIMER0 = 48Mhz
	// Thiet lap Mode=OneShot & Presscaler=0
	TIMER0->TCSR = TIMER_ONESHOT_MODE ;
  TIMER0->TCMPR = 480;										// 480 -> 10us
  TIMER_EnableInt(TIMER0);
  NVIC_EnableIRQ(TMR0_IRQn);
	
	// Cau hinh Timer 1 de tao xung Rising, Timer1 Period Mode & Prescaler=2
	TIMER_Open(TIMER1, TIMER_PERIODIC_MODE, TMR1_OPERATING_FREQ);
	TIMER0->TCSR = TMR1_OPERATING_MODE | 2;
  TIMER_EnableInt(TIMER1);
  NVIC_EnableIRQ(TMR1_IRQn);

  // Configure Timer 1: Stepper Driver Interrupt	
//  TCCR1B &= ~(1<<WGM13); // waveform generation = 0100 = CTC	//  Thiet lap timer hoat dong o che do CTC (Clear Timer Compare) thong so dua vao OCR1A 
//  TCCR1B |=  (1<<WGM12);
//  TCCR1A &= ~((1<<WGM11) | (1<<WGM10));
//  TCCR1A &= ~((1<<COM1A1) | (1<<COM1A0) | (1<<COM1B1) | (1<<COM1B0)); // Disconnect OC1 output // Phuc: Khong ket noi voi chan OC1A va OC1B 
